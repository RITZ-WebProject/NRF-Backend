...
<html lang="en">
  <head>
    ...
    <link
      rel="stylesheet"
      href="{{asset('admin/vendors/jsgrid/jsgrid.min.css')}}"
    />
    <link
      rel="stylesheet"
      href="{{asset('admin/vendors/jsgrid/jsgrid-theme.min.css')}}"
    />
    --}}
                  ...
                  $(document).on('change', '.status-select', function() {
                      ...
                      $.ajax({
                          ...
                          success: function(data) {
                              // console.log(data);
                              // handle success
                              // $('#status-' + orderId).text(data.status);
                              // $('#message-' + orderId).text(data.message);
          					...
          					if (newStatus === "success") {
                                ...
                                badge="info";
                              	...
                              	badge="danger";
                        //increaseProductQuantity(orderId);
          					}
                          	// console.log(orderId);
                          	// console.log(newStatus);
                              // console.log($('#badge-' + orderId).text(data.status));
      						var newSpan = '<span class="badge badge-pill badge-' + badge + '">' + filteredData + '</span>';

      						var badge_name='#badge-' + orderId;
          					// Replace the existing span element with the new one
          					$(badge_name + ' span').replaceWith(newSpan);

                          	$('.' + orderId).val(newStatus)
                          	// var status_name='#status-'+ orderId;

                          	// console.log(status_name);
							// var count = $('[id="status-NRF-O-000250"]').length;
							// console.log(count);

                          	// $('#status-NRF-O-000250').val(newStatus);
							// $(status_name).val(newStatus);


      // $(status_name).find('option:selected').removeAttr('selected');
      //       $(status_name).find('option[value="cancelled"]').prop('selected', true);
                          	// $(status_name).val(newStatus);

//                           	$('#order-listing tbody tr').each(function() {
//               					var rowId = $('#order_uniquekey').text();
//               					if (rowId === orderId) {
//                                 console.log(rowId);
//                 						console.log('sup');
//                                 // console.log('#status-' + rowId);
//                                   	$('#status-' + rowId).val(newStatus);
//               					}
...
//             					});
                          	...
                          	Swal.fire({
        							...
        							// text: 'This is a simple sweet alert box.',
        							icon: 'success',
			...
			$(document).ready(function() {
                  $(document).on('change', '.pstatus-select', function() {
                      ...
                      $.ajax({
                          ...
                          success: function(data) {
                            //   console.log(data);
                              // $('#status-' + productId).text(data.status);
                              // $('#message-' + productId).text(data.message);
                          	Swal.fire({
