...
class OrderController extends Controller
{
    ...
    public function send_cancel_email(Request $request) {
        $order = Order::findOrFail($request->order_id);
        $customerEmail = $order->customer->email;
        $customerName = $order->customer->name;
        Mail::to($customerEmail)->send(new OrderCancelled($customerName));
        return response()->json(['success' => true]);
    }
	...
	public function update_order_status(Request $request){
        ...
        try {
            ...
            $customerEmail = $order->customer->email;
            $customerName = $order->customer->name;
            $products = $order->product->pluck('product_name')->toArray();
             Mail::to($customerEmail)->queue(new OrderConfirmed($customerName));
        } catch (\Exception $e) {
            return response()->json(['status' => 'error', 'message' => 'Failed to send confirmation email']);
            }
        }
    	...
    	if($request->status == 'cancelled'){
            ...
            $customerEmail = $order->customer->email;
            $customerName = $order->customer->name;
            Mail::to($customerEmail)->queue(new OrderCancelled($customerName));
